...
Model Picker runs as a trusted Codex hook, reads an API key, and sends prompt
excerpts to TypeSafe. That trust surface gets a written policy.
...
3. Only allowlisted model and effort labels and fixed text enter the Codex
   model context. Prompt excerpts and provider output never do.
4. Session state stays under the state directory with mode 0600, whatever the
   session ID contains.
5. The hook never changes the Codex model, effort, config or other hooks, and
   never reads source files or the transcript.
6. `advisor.py` imports only the Python standard library. CI checks this.
